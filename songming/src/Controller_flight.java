...
public class Controller_flight {
	...
	public void action(int selectedIndex, Flight_view view, Controller_mainmenu c,int row,int seatNum) {
		...
		this.from = c.getFrom();
		this.to = c.getTo();
		this.seat = airplane.getSeatClassList().get(c.getSeatClass()).getRow().get(row).getRowNumber()+airplane.getSeatClassList().get(c.getSeatClass()).getRow().get(row).getSeat().get(seatNum).getSeatNum();
		this.name = c.getName();
		this.date = c.getDate();
